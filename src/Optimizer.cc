...
int Optimizer::PoseOptimization(Frame* pFrame)
{
    ...
    const float deltaStereo = sqrtf(7.815f);
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pKF, bool* pbStopFlag, Map* pMap)
{
    ...
    for (auto pKFi : lLocalKeyFrames) {
        ...
        for (const auto& pLandmark : landmarks) {
            ...
            pKFi->landmarkMeasurements[pLandmark->mnLandmarkId] = pLandmark->GetCuboid()->transformTo(pKFi->cam_pose_Twc);
        }
        auto* vSE3 = new g2o::VertexSE3Expmap();
            ...
            if (!pKFi->isBad()) {
                ...
                if (pKFi->mvuRight[mit->second] < 0) {
                    ...
                    for (const auto& pLandmark : landmarks) {
                        ...
                        e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(offset + 1 + pLandmark->mnLandmarkId)));
    ...
    //Landmarks
    for (const auto& pLandmark : lLocalLandmarks) {
        g2o::VertexCuboid* vCube = static_cast<g2o::VertexCuboid*>(optimizer.vertex(offset + 1 + pLandmark->mnLandmarkId));
                                       ...
                                       const map<KeyFrame*, set<KeyFrame*> >& LoopConnections, const bool& bFixScale)
{
    ...
    auto linearSolver = g2o::make_unique<g2o::LinearSolverEigen<g2o::BlockSolver_7_3::PoseMatrixType>>();
    auto solver_ptr = g2o::make_unique<g2o::BlockSolver_7_3>(std::move(linearSolver));
    g2o::OptimizationAlgorithmLevenberg* solver = new g2o::OptimizationAlgorithmLevenberg(std::move(solver_ptr));
        ...
        for (vector<KeyFrame*>::const_iterator vit = vpConnectedKFs.begin(); vit != vpConnectedKFs.end(); vit++) {
            ...
            if (pKFn && pKFn != pParentKF && !pKF->hasChild(pKFn) && !sLoopEdges.count(pKFn)) {
                if (!pKFn->isBad() && pKFn->mnId < pKF->mnId) {
                    ...
                    en->setVertex(1, optimizer.vertex(pKFn->mnId));
